        ...
        >
          <a-row type="flex" justify="space-between">
            ...
            >
              ...
              />
              <a-divider></a-divider>
              <router-link to="/account">User Panel</router-link>
            </a-card>
            <a-card
              :loading="loading"
              :style="{ width: '250px', marginBottom: '15px' }"
            >
              <a-statistic
                title="Running Jobs"
                :value-style="{ color: '#b1b100' }"
                :value="data.job.running"
              />
              <a-statistic
                title="Online Controllers"
                :style="{ marginTop: '10px' }"
                :value="data.controller.online"
                :value-style="{ color: '#3f8600' }"
              >
                <template #suffix
                  ><span> / {{ data.controller.total }}</span></template
                >
              </a-statistic>
              <a-divider></a-divider>
              <router-link to="/config/index">Config Panel</router-link>
            </a-card>
            <a-card
              :loading="loading"
              :style="{ width: '250px', marginBottom: '15px' }"
            >
              <a-statistic
                title="Online Nodes"
                :value="data.node.online"
                :value-style="{ color: '#0073e4' }"
              />
              <a-statistic
                title="Total Nodes"
                :value="data.node.total"
                :style="{ marginTop: '10px' }"
              />
              <a-divider></a-divider>
              <router-link to="/node">Node Panel</router-link>
            </a-card>
            <a-card
              :loading="loading"
              :style="{ width: '250px', marginBottom: '15px' }"
            >
              <a-statistic
                ...
                suffix="Mbps"
                :value="data.bandwidth.throughput"
                :value-style="{ color: '#cb0001' }"
              />
              <a-statistic
                title="Design Throughput"
                suffix="Mbps"
                :value="data.bandwidth.design"
                :style="{ marginTop: '10px' }"
              />
              <a-divider></a-divider>
              <router-link to="/cluster">Cluster Panel</router-link>
            </a-card>
          </a-row>
          <a-row type="flex" justify="space-between">
            <a-col span="16">
              <a-card
                :loading="loading"
                title="Number of Online Users and Running Jobs in the Past 7 Days"
